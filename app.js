...
class RestaurantApp {
  ...
  getTotal() {
    ...
    const deliveryCharge =
      this.currentCoupon && this.currentCoupon.target === "delivery" ? 0 : 40;
  ...
  handleFilter(button) {
      ...
      case "cost-low":
        sortedCards.sort((a, b) => {
          const costA = parseInt(
            a.querySelector(".cost").textContent.match(/\d+/)[0],
          );
          const costB = parseInt(
            b.querySelector(".cost").textContent.match(/\d+/)[0],
          );
          return costA - costB;
        });
        break;
    ...
    if (countText) {
      const visibleCount = sortedCards.filter(
...
function handleSearch() {
  ...
  document.querySelectorAll(".res-card").forEach((card) => {
    ...
    const cuisine = card
      .querySelector(".res-cuisine")
      .textContent.toLowerCase();
    card.style.display =
      name.includes(input) || cuisine.includes(input) ? "block" : "none";
  });
}
